...
serial_manager_status_t Serial_UsbCdcInit(serial_handle_t serialHandle, void *serialConfig)
{
    ...
    USB_DEBUG_PRINTF("Entering Serial_UsbCdcInit...\n");
    assert(serialConfig);
    assert(serialHandle);
    ...
    USB_DEBUG_PRINTF("USB_DeviceClassInit successful...\n");

    USB_DeviceAddItem(&s_UsbCdcHead, serialUsbCdc);
    USB_DEBUG_PRINTF("USB_DeviceAddItem completed...\n");

    USB_DeviceIsrEnable(serialUsbCdc);
    USB_DEBUG_PRINTF("USB_DeviceIsrEnable completed...\n");

    /*Add one delay here to make the DP pull down long enough to allow host to detect the previous disconnection.*/
    SDK_DelayAtLeastUs(5000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    USB_DEBUG_PRINTF("SDK_DelayAtLeastUs completed...\n");
    USB_DeviceRun(serialUsbCdc->deviceHandle);
    USB_DEBUG_PRINTF("USB_DeviceRun completed...\n");

    USB_DEBUG_PRINTF("Serial_UsbCdcInit: returning [%d]\n", kStatus_SerialManager_Success);
