    ...
    uint8_t controllerId, const usb_device_controller_interface_struct_t **controllerInterface)
{
    ...
    usb_controller_index_t controllerIndex = (usb_controller_index_t)controllerId;

    USB_DEBUG_PRINTF("Entering USB_DeviceGetControllerInterface... controllerIndex=%d\n", controllerId);
#if ((defined(USB_DEVICE_CONFIG_KHCI)) && (USB_DEVICE_CONFIG_KHCI > 0U))
    /* Get the KHCI controller driver interface */
    if ((kUSB_ControllerKhci0 == controllerIndex) || (kUSB_ControllerKhci1 == controllerIndex))
    {
    ...
    if ((kUSB_ControllerEhci0 == controllerIndex) || (kUSB_ControllerEhci1 == controllerIndex))
    {
     ...
     ((defined(USB_DEVICE_CONFIG_LPCIP3511HS)) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U)))
    /* Get the EHCI controller driver interface */
    if ((kUSB_ControllerLpcIp3511Fs0 == controllerIndex) || (kUSB_ControllerLpcIp3511Fs1 == controllerIndex) ||
        (kUSB_ControllerLpcIp3511Hs0 == controllerIndex) || (kUSB_ControllerLpcIp3511Hs1 == controllerIndex))
    {
...
#if ((defined(USB_DEVICE_CONFIG_DWC3)) && (USB_DEVICE_CONFIG_DWC3 > 0U))
    /* Get the EHCI controller driver interface */
    if ((kUSB_ControllerDwc30 == controllerIndex) || (kUSB_ControllerDwc31 == controllerIndex))
    {
    ...
    USB_DEBUG_PRINTF("USB_DeviceGetControllerInterface: returning [%d]\n", error);
    return error;
}
...
usb_status_t USB_DeviceInit(uint8_t controllerId, usb_device_callback_t deviceCallback, usb_device_handle *handle)
{
    ...
    for (count = 0U; count < (USB_DEVICE_CONFIG_ENDPOINTS * 2U); count++)
    {
    	USB_DEBUG_PRINTF("USB_DeviceInit: initializing endpoint [%d]\n", count);
	...
	USB_DEBUG_PRINTF("USB_DeviceInit: checking device controller functions....\n", count);
    ...
    *handle = deviceHandle;
	...
	USB_DEBUG_PRINTF("USB_DeviceInit: initializing controller... [0x%08X]\n", &(deviceHandle->controllerInterface->deviceInit));
