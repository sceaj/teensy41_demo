...
{
    USB_DEBUG_PRINTF("Entering USB_EhciGetValidEhciState...\n");
    for (uint8_t instance = 0; instance < USB_DEVICE_CONFIG_EHCI; instance++)
    {
        if (0U == g_UsbDeviceEhciStateStatus[instance])
        {
            ...
            *instanceIndex                       = instance;
            USB_DEBUG_PRINTF("USB_EhciGetValidEhciState: returning from instance %d\n", instance);
            return (void *)(&g_UsbDeviceEhciState[instance]);
        }
    }
    USB_DEBUG_PRINTF("USB_EhciGetValidEhciState: returning NULL\n");
    return NULL;
}
                                ...
                                usb_device_controller_handle *ehciHandle)
{
    ...
    void *temp;
    USB_DEBUG_PRINTF("Entering USB_DeviceEhciInit...\n");
#if (defined(USB_DEVICE_CONFIG_CHARGER_DETECT) && (USB_DEVICE_CONFIG_CHARGER_DETECT > 0U)) && \
    ((defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && (FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U)) ||    \
     (defined(FSL_FEATURE_SOC_USB_ANALOG_COUNT) && (FSL_FEATURE_SOC_USB_ANALOG_COUNT > 0U)))
    ...
    {
        USB_DEBUG_PRINTF("USB_DeviceEhciInit failed: [%d]\n", kStatus_USB_ControllerNotFound);
    ...
    USB_DEBUG_PRINTF("USB_DeviceEhciInit: ehciState->registerBase has been set: 0x%08X\n", ehciState->registerBase);
#if (defined(USB_DEVICE_CONFIG_LOW_POWER_MODE) && (USB_DEVICE_CONFIG_LOW_POWER_MODE > 0U))
    ...
    USB_DEBUG_PRINTF("USB_DeviceEhciInit: resetting the controller...\n");
//    enterMonitor();
    /* Reset the controller. */
    USB_DEBUG_PRINTF("USB_DeviceEhciInit: USBCMD [0x08X] before\n", ehciState->registerBase->USBCMD);
    ehciState->registerBase->USBCMD |= USBHS_USBCMD_RST_MASK;
    USB_DEBUG_PRINTF("USB_DeviceEhciInit: USBCMD [0x08X] after\n", ehciState->registerBase->USBCMD);
    uint32_t resetCount = 0;
    while (0U != (ehciState->registerBase->USBCMD & USBHS_USBCMD_RST_MASK))
    {
    	resetCount++;
    	if ((resetCount % 1000000U) == 0) {
    		USB_DEBUG_PRINTF("USB_DeviceEhciInit: waiting for reset [0x%08X]\n", ehciState->registerBase->USBCMD);
    	}
    }
    ...
    {
    	USB_DEBUG_PRINTF("USB_DeviceEhciInit failed: Expected %d endpoints, but only found %d.", USB_DEVICE_CONFIG_ENDPOINTS, ehciState->endpointCount);
        USB_DEBUG_PRINTF("USB_DeviceEhciInit: returning [%d]\n", kStatus_USB_Error);
        return kStatus_USB_Error;
    }
