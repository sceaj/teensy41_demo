...
)
{
    ...
    uint8_t mapIndex;
    uint8_t classIndex;
    USB_DEBUG_PRINTF("Entering USB_DeviceClassInit...\n");
    ...
    USB_DEBUG_PRINTF("USB_DeviceInit: [%d]\n", error);
    assert(kStatus_USB_Success == error);
    if (kStatus_USB_Success != error)
    {
        (void)USB_DeviceDeinit(classHandle->handle);
        USB_DEBUG_PRINTF("USB_DeviceDeinit completed...\n");
        (void)USB_DeviceClassFreeHandle(controllerId);
        USB_DEBUG_PRINTF("USB_DeviceClassFreeHandle completed...\n");
        return error;
    }
    ...
    for (classIndex = 0U; classIndex < classHandle->configList->count; classIndex++)
    {
        USB_DEBUG_PRINTF("USB Class initialization: classIndex = %d\n", classIndex);
        for (mapIndex = 0U; mapIndex < (sizeof(s_UsbDeviceClassInterfaceMap) / sizeof(usb_device_class_map_t));
             mapIndex++)
        {
            USB_DEBUG_PRINTF("USB Class initialization: mapIndex = %d\n", mapIndex);
                ...
                s_UsbDeviceClassInterfaceMap[mapIndex].type)
            {
                ...
                USB_DEBUG_PRINTF("USB Class initialized: classIndex = %d mapIndex = %d\n", classIndex, mapIndex);
            }
        }
    }

    *handle = classHandle->handle;
    USB_DEBUG_PRINTF("USB_DeviceClassInit: returning [%d]\n", error);
    return error;
}
